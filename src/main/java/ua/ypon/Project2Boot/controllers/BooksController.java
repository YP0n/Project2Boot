package ua.ypon.Project2Boot.controllers;
...
import ua.ypon.Project2Boot.services.BookService;
import ua.ypon.Project2Boot.services.PeopleService;
...
public class BooksController {
    ...
    @PostMapping()//valid- перевірка умов. BindingResult-об"єкт для збереження помилок при
    // створенні Book і введенні неValid.
    public String create(@ModelAttribute("book") @Valid Book book,
                         BindingResult bindingResult) {//за допомогою @ModelAttribute створюємо нову
        // книгу(пустий об"єкт класа Book)

        if (bindingResult.hasErrors())
            return "books/new";

        bookService.save(book);//потім ця ж анотація впроавджує значення із форми
        // в цей об"єкт класа Book. І цей об"єкт додає в модель.
        return "redirect:/books";
    }

    //Метод "edit(Model model, @PathVariable("id") int id)" обробляє GET-запит на шлях "/books/{id}/edit".
    // Він отримує об'єкт "Book" з сервісу "BookService" за заданим ідентифікатором і
    // передає його у модель для використання при редагуванні особи у представленні "books/edit".
    @GetMapping("/{id}/edit")
    ...
    //Метод "update(@ModelAttribute("book") @Valid Book book, BindingResult bindingResult,
    // @PathVariable("id") int id)" обробляє PATCH-запит на шлях "/books/{id}".
    // Він отримує об'єкт "Book" з форми, проводить валідацію та оновлює існуючий об'єкт "Book"
    // з новими даними у базі даних за допомогою сервісу "BookService".
    @PatchMapping("/{id}")
